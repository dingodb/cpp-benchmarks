...
static void DoSetup_for_EmptyDb(const benchmark::State& state) {
  rocks_oper = dingodb::RocksTxnOper::get_instance(
      kDBPath,
      kDB_cfg_path);
  txn_db = rocks_oper->get_rocksdb();
  assert(nullptr != txn_db);
}
...
static void BM_rocksdb_put(benchmark::State& state) {
  ...
  for (auto _ : state) {
      ...
      s = txn_db->Put(WriteOptions(), key, val);
      assert(s.ok());
  }
}
...
static void BM_rocksdb_get(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    std::string val;
    s = txn_db->Get(ReadOptions(), key, &val);
    assert(s.ok());
  }
}
...
static void BM_rocksdb_scan(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    ReadOptions rdOpts;
    rdOpts.snapshot = txn_db->GetSnapshot();
    rdOpts.readahead_size = state.range(1);
    std::unique_ptr<Iterator> iter(txn_db->NewIterator(rdOpts));
    iter->Seek(key);
    size_t count = 0;
    for (int i = 0; i < scanRange; i++) {
      std::string val;
      s = txn_db->Get(rdOpts, key, &val);
      assert(s.ok());
